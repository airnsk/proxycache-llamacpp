...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    if (ex == LLAMA_EXAMPLE_SERVER) {
        ...
        add_opt(common_arg(
            {"--decision-prefixes"}, "N",
            string_format("static prefixes the decision engine keeps resident, LRU-evicted; -1 = auto, half of the --decision-seqs budget (default: %d)", params.n_decision_prefixes),
            [](common_params & params, int value) {
                if (value != -1 && value < 1) {
                    throw std::invalid_argument("--decision-prefixes takes N >= 1, or -1 for auto");
                }
