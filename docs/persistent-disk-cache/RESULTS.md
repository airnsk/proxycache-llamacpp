...
- **I/O под общим мьютексом (Stage 6).** Проверено: дисковый кэш дёргается только из главного цикла
  ...
  выигрыша не даёт.
- **Сервер встаёт на время restore.** Холодный restore 4.2 ГиБ — ≈25 s, и это синхронная работа в
  главном цикле. Чтобы слоты не ждали, нужен либо промежуточный буфер в RAM (конфликтует с
  требованием «не хранить multi-GB состояние в RAM»), либо порционная загрузка — но API состояний
  работает целиком (`llama_state_seq_load_file` / `llama_state_seq_set_data_ext`, частичного
  применения нет). Не реализовано, решение открыто.
