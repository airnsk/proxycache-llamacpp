...
struct llama_model_qwen4exp : public llama_model_base {
    ...
    struct graph : public llm_build_delta_net_base {
        ...
        // base for graph_mtp: binds the model without building the main graph
        graph(const llama_model & model, const llm_graph_params & params, bool /*mtp_stub*/) :
  ...
  const llama_memory_hybrid_idx_context * mctx_hyb);
    ...
    // draft MTP head: one extra qwen4exp block plus the nextn fusion and its own HC mixer
    struct graph_mtp : public graph {
        graph_mtp(const llama_model & model, const llm_graph_params & params);
    };
